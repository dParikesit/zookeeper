...
public class DataTree {
    ...
    /**
     * create a /zookeeper/config node for maintaining the configuration (membership
     * and quorum system) info for
     * zookeeper
     */
     ...
     *                       the session id that owns this node. -1 indicates this
     *                       is not
     *                       an ephemeral node.
     * @param zxid
     *                       Transaction ID
     ...
     */
    public void createNode(final String path, byte[] data, List<ACL> acl, long ephemeralOwner, int parentCVersion,
            long zxid, long time, Stat outputStat) throws NoNodeException, NodeExistsException {
    ...
    public ProcessTxnResult processTxn(TxnHeader header, Record txn, boolean isSubTxn) {
        ...
        LOG.info("DIMAS: dataTree " + header.getType());
    ...
    void serializeNode(OutputArchive oa, StringBuilder path) throws IOException {
        ...
        int off = path.length();
        for (String child : children) {
            // Since this is single buffer being reused, we need to truncate the previous
            // bytes of string.
     ...
     *
     * @param path
     *                    Full path to the znode whose Cversion needs to be
     *                    modified.
    ...
    /**
     * Serializes the lastProcessedZxid so we can get it from snapshot instead the
     * snapshot file name.
     * This is needed for performing snapshot and restore via admin server commands.
     *
     * @param oa the output stream to write to
     * @return true if the lastProcessedZxid is serialized successfully, otherwise
     *         false
    ...
    /**
     * Deserializes the lastProcessedZxid from the input stream and updates the
     * lastProcessedZxid field.
     *
     * @param ia the input stream to read from
     * @return true if lastProcessedZxid is deserialized successfully, otherwise
     *         false
     * @throws IOException if there is an I/O error
     */
     ...
     *
     * @return false if digest in the txn doesn't match what we have now in the data
     *         tree
     */
